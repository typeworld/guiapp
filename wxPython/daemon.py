...
try:
	...
	if MAC:
		if appIsRunning('world.type.agent'):
			log('Quitting second instance')
			quit()
	...
	def getAmountOutdatedFonts(force = False):
		...
		if appIsRunning(ID):
			...
			conn = Client(address)
			if force:
				conn.send('amountOutdatedFonts force')
			else:
				conn.send('amountOutdatedFonts')
			amountOutdatedFonts = conn.recv()
	...
	#				amountOutdatedFonts = conn.recv()
				...
				if MAC:
					log('about to start app with amountOutdatedFonts command')
					call = '"%s" amountOutdatedFonts' % os.path.join(mainAppPath, 'Contents', 'MacOS', 'Type.World')
					log('call: %s' % call)
					response = Execute(call).strip()
					log(response)
					amountOutdatedFonts = int(response)

		log('amountOutdatedFonts: %s' % amountOutdatedFonts)

		return amountOutdatedFonts
