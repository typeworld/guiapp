	...
	class TypeWorldApp(object):
		...
		def notification(self, text, title = None):
			...
			if WIN:
				...
				template.setFirstLine(text)
				expiration = 24 * 60 * 60 * 1000 # one day
				template.setExpiration(expiration) # One day
				template.addAction(title)
				notificationID = zroya.show(template) # , on_action=onAction


		def path(self):
			if MAC:
				return str(NSWorkspace.sharedWorkspace().URLForApplicationWithBundleIdentifier_('world.type.guiapp'))[7:]
			if WIN:
				return os.path.abspath(os.path.join(os.path.dirname(__file__), 'TypeWorld.exe'))
			...
			if self.isOpen():
				...
				if self.path():
					...
					if MAC:
						...
						call = '"%s" %s' % (os.path.join(self.path(), 'Contents', 'MacOS', 'Type.World'), command)
						log('call: %s' % call)
		...
		def open(self):
			...
			if MAC:
				# App is running, so activate it
				apps = list(NSRunningApplication.runningApplicationsWithBundleIdentifier_(self.ID()))
				if apps:
					mainApp = apps[0]
				...
				# Not running, launch it
					...
					NSWorkspace.sharedWorkspace().launchAppWithBundleIdentifier_options_additionalEventParamDescriptor_launchIdentifier_(self.ID(), 0, None, None)
